...
def feature_channels(use_z: bool, use_velocity: bool, use_acceleration: bool = False) -> int:
    c = 3 if use_z else 2
    if use_acceleration: return c * 3
    if use_velocity: return c * 2
    return c
...
def create_detector(task_model_path: Path, det_conf: float, track_conf: float):
    # Dùng chuẩn Tasks API của Google mới nhất
    ...
    options = HandLandmarkerOptions(
        ...
        # 🔥 ĐÃ XÓA dòng min_hand_presence_confidence để AI tự reset khi bị lỗi cục diện
    )
...
def main() -> None:
    ...
    parser.add_argument("--det-conf", type=float, default=0.6) # 🔥 Ép nhạy kịch kim
    parser.add_argument("--track-conf", type=float, default=0.35) # 🔥 Ép nhạy kịch kim
    ...
    parser.add_argument("--missing-reset-frames", type=int, default=30) # Chống mất trí nhớ
    ...
    model_candidates = [
        ...
        Path("outputs/outputs/stgcn_best.pt"), Path("outputs/stgcn_best.pt"), Path("data/stgcn_best.pt")
    ]
    labels_candidates = [
        Path("outputs_resume2/labels.json"), Path("outputs_resume/labels.json"),
        Path("outputs/outputs/labels.json"), Path("outputs/labels.json"), Path("data/labels.json")
    ]
    ...
    if model_path is None or labels_path is None: raise FileNotFoundError("Missing checkpoint or labels!")
    ...
    double_click_cooldown, last_double_type = 0, None

    print(f"✅ Đã kết nối với Não 85.7%: {model_path} | Channels: {in_channels}")
        ...
        while True:
            ...
            if landmarks is not None:
                ...
                if missing_count >= args.missing_reset_frames:
                    ...
                    # 🔥 Đã vá: Nếu rớt tracking tạm thời, lấy frame cũ đắp vào để khỏi gãy sequence
            ...
            if len(frame_buffer) == args.length:
                ...
                if prob_ema is None: prob_ema = probs
                else: prob_ema = args.ema_alpha * prob_ema + (1.0 - args.ema_alpha) * probs
                ...
                if top_conf >= args.min_confidence:
                    display, color = f"{top_text} ({top_conf:.2f})", (30, 230, 30)
                else:
                    display, color = f"Khong chac ({top_conf:.2f})", (40, 170, 250)
